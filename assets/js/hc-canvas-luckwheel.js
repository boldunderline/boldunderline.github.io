(function () {
  ...
  function draw(opts) {
    ...
    const curveFontSizeClass = num < 80 ? "fs-2vh" : "fs-1.2vh";
    ...
    if (!canvas.getContext) {
      showMsg("Browser is not supported, try downloading latest version of chrome");
      return;
    }
    ...
    for (let i = 0; i < num; i++) {
      ...
      if (opts.mode == "both") {
        ...
        html.push(`<p class="curve ${curveFontSizeClass}">` + prizeList[i].text + "</p>");
      }
      html.push("</span> </li>");
      ...
      if (i + 1 === num) {
        ...
        prizeItemsContainer && container.removeChild(prizeItemsContainer);
  ...
  function eGot() {
    ...
    if (optsPrize.chances == null) {
      ...
      return fnGotBack(prizes[optsPrize.prizeId], optsPrize.prizeId);
    }
  }
